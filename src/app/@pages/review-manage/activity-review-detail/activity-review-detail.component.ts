...
export class ActivityReviewDetailComponent extends BaseComponent implements OnInit {
  ...
  constructor(storageService: StorageService, private router: Router, private dialogService: DialogService) {
    ...
    if(!!this.router.getCurrentNavigation()?.extras){
      ...
      this.oldReview.activityReviewHistory.forEach(history => {
        if(!this.historyGroupView || !this.historyGroupView[history.groupId]){
          ...
          this.historyGroupView[history.groupId].flows.push({time: history.time, title: history.title, detail: history.detail});
        }
      });
    }
  }
